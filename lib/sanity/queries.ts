...
export async function getSiteSettings(locale: Locale = 'en') {
  return client.fetch(`*[_type == "siteSettings"][0]{
    ...
    "role": ${localized('role', locale)},
    "heroHeadline": ${localized('heroHeadline', locale)},
    "heroAccent": ${localized('heroAccent', locale)},
    "heroLede": ${localized('heroLede', locale)},
    "aboutBio": ${localized('aboutBio', locale)},
    "resumeIntro": ${localized('resumeIntro', locale)},
    resumeFile{asset->{url}},
    email, linkedin, github,
    "contactHeading": ${localized('contactHeading', locale)},
    "contactBody": ${localized('contactBody', locale)},
...
export async function getProjects(locale: Locale = 'en') {
  return client.fetch(`*[_type == "project" && draft != true] | order(order asc){
    ...
    "slug": slug.current,
    "summary": ${localized('summary', locale)},
    image, link, tags, featured,
    "category": category->{"title": ${localized('title', locale)}, "slug": slug.current}
  }`);
}
...
export async function getExperience(locale: Locale = 'en') {
  return client.fetch(`*[_type == "experience"] | order(order asc){
    "role": ${localized('role', locale)},
    "organization": ${localized('organization', locale)},
