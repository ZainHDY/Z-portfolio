...
export async function generateMetadata({ params }: { params: { locale: Locale } }): Promise<Metadata> {
  ...
  return { title, description, alternates: { canonical: url, languages: { en: `${siteUrl}/en`, ar: `${siteUrl}/ar` } }, openGraph: { title, description, url, type: 'website', locale: locale === 'ar' ? 'ar' : 'en_US' }, twitter: { card: 'summary', title, description }, icons: brandMarkUrl ? { icon: brandMarkUrl, shortcut: brandMarkUrl, apple: brandMarkUrl } : undefined };
}
